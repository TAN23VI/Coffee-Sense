...
namespace CoffeeShopManagement
{
    class Program
    {
        ...
        static List<int> availableStock = Enumerable.Range(1, 50).ToList();
        static List<MENU> MENU = new List<MENU>
        {
            new MENU { Id = 1, TypeName = "Drinks" },
            new MENU { Id = 2, TypeName = "Hot Beverages" },
            new MENU { Id = 3, TypeName = "Cold Beverages" },
            new MENU { Id = 4, TypeName = "Customer Favourite" },
        };
            ...
            while (n < 4)
            {
                ...
                switch (ch)
                {
                    case "1":
                        ...
                        {
                            Console.WriteLine($"You have selected: {selectedMENU.TypeName}");
                            CoffeeTransaction coffeeTransaction = new CoffeeTransaction(confirmationService, availableCoffee, selectedCoffee);
                            transactions.AddRange(coffeeTransaction.BuyCoffee());
                        }
                        break;
                    case "2":
                        can.ReturnCoffee(transactions);
                        break;
                    case "3":
                        n = 4;
                        break;
